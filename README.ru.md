...
| Команда | Назначение |
|---|---|
| `app:build` | Записывает кеши конфигурации и контейнера, собирает известные корни автоматической сборки и создаёт лениво подключаемые шарды фабрик с именами по содержимому. Команда должна запускаться с `APP_ENV=development`. |
...
Discovery финализируется один раз, после чего все listener compilers и зарегистрированные cache contributors получают один и тот же снимок исходных данных. Пустые массивы, `null` и стандартные значения `false` не записываются. Обязательный маркер версии, например пустая CQRS map v2 `['version' => 2]`, сохраняется, потому что это не пустая секция. Build-only реестры и `#[DevOnly]` listeners удаляются из production-конфигурации; если runtime-listeners не осталось, таблица discovery-классов также не записывается.

Скомпилированные фабрики записываются в обычную секцию `dependencies.factories`, причём явно настроенные фабрики приложения имеют приоритет. Шарды подключаются при первом обращении, SHA-256 исходников во время запуска не вычисляется. Повторяйте `app:build` после изменений провайдеров, обнаруживаемых PHP-классов, маршрутов, метаданных CQRS, расширений DI или зависимостей развертывания.
